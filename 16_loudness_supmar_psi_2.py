...
ton_conds = ['negative/r1','negative/r2','negative/s1','negative/s2', 'positive/r1','positive/r2','positive/s1','positive/s2']
...
for meg, mri in sub_dict.items():
    ...
    # data_cov = mne.compute_covariance(epo[ton_conds], tmin=None, tmax=None, method='empirical',n_jobs=8)
    # noise_cov = mne.compute_covariance(epo['rest'], tmin=None, tmax=None, method='empirical',n_jobs=8)
    ...
    # write_inverse_operator('{}{}_vrest-oct6-inv.fif'.format(proc_dir,meg), inv_op)
    # del fwd, data_cov, noise_cov
    # read inverse operator
    inv_op = read_inverse_operator('{}{}_vrest-oct6-inv.fif'.format(proc_dir,meg))
    # then apply the inverse to get source activity
    stcs = apply_inverse_epochs(epo[ton_conds], inv_op, lambda2, method, pick_ori=None, return_generator=True)
    # read labels
    ...
    label = [l for l in labels if l.name == 'supramarginal-lh']
    del labels
    # calculate label timecourse for seed region
    seed_ts = mne.extract_label_time_course(stcs, label, inv_op['src'], mode='pca_flip')
    ...
    psi, freqs, times, n_epochs, n_tapers = phase_slope_index(comb_ts, mode='multitaper', indices=indices, mt_bandwidth = 10.,
...
# GA_mix_stc.save("{}GA_fs_mix_Loud_SM_PSI_stc.h5".format(proc_dir))
GA_surf_stc = GA_mix_stc.surface()
GA_surf_stc.save("{}GA_fs_surf_Loud_SM_PSI_2_gamma_stc.h5".format(save_dir))
...
label = [l for l in labels if l.name == 'supramarginal-lh'][0]
...
GA_hd_surf.save("{}GA_fs_HD_surf_Loud_SM_PSI_2_gamma_stc.h5".format(save_dir))
...
GA_ld_surf.save("{}GA_fs_LD_surf_Loud_SM_PSI_2_gamma_stc.h5".format(save_dir))
brain = GA_diff.plot(surface='inflated', hemi='lh', time_label='Phase Slope Index (PSI)',
                        subjects_dir=mri_dir, src=fs_surf, clim=dict(kind='percent', pos_lims=(95, 97.5, 100)),
                        time_viewer=True, show_traces=False)
brain = GA_hd_surf.plot(surface='inflated', hemi='lh', time_label='Phase Slope Index (PSI)',
                        subjects_dir=mri_dir, src=fs_surf, clim=dict(kind='value', pos_lims=(0.003, 0.004, 0.005)),
                        time_viewer=True, show_traces=False)
brain.add_label(label, color='green', alpha=0.5)
brain.add_annotation('aparc', borders=1, alpha=0.8)
