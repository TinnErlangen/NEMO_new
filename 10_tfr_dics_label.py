...
for meg,mri in sub_dict.items():
    ...
    fig_n = plot_source_spectrogram(stcs_neg, freq_bins, tmin=tmin_plot, tmax=tmax_plot, source_index=None, colorbar=True, show=False)    # freq_bins! or error
    ...
    fig_p = plot_source_spectrogram(stcs_pos, freq_bins, tmin=tmin_plot, tmax=tmax_plot, source_index=None, colorbar=True, show=False)
    ...
    fig_c = plot_source_spectrogram(stcs_diff, freq_bins, tmin=tmin_plot, tmax=tmax_plot, source_index=None, colorbar=True, show=False)
    fig_c.savefig("{}{}_cont_N-P_{}.png".format(plot_dir,meg,loi))

# GA for negatives
for f in range(len(coll_stcs_neg[0])):
    GA_stcs_neg.append(coll_stcs_neg[0][f])
    for s in range(len(coll_stcs_neg[1:])):
        GA_stcs_neg[f] += coll_stcs_neg[s][f]
    GA_stcs_neg[f] /= len(coll_stcs_neg)
for fb in range(len(freq_bins)):
    GA_stcs_neg[fb].save("{}GA_TF_dics_neg_{}-{}_{}-stc.h5".format(save_dir,freq_bins[fb][0],freq_bins[fb][-1],loi))
GA_fig_n = plot_source_spectrogram(GA_stcs_neg, freq_bins, tmin=tmin_plot, tmax=tmax_plot, source_index=None, colorbar=True, show=False)    # freq_bins! or error
GA_fig_n.savefig("{}GA_neg_{}.png".format(plot_dir,loi))

# GA for positives
for f in range(len(coll_stcs_pos[0])):
    GA_stcs_pos.append(coll_stcs_pos[0][f])
    for s in range(len(coll_stcs_pos[1:])):
        GA_stcs_pos[f] += coll_stcs_pos[s][f]
    GA_stcs_pos[f] /= len(coll_stcs_pos)
for fb in range(len(freq_bins)):
    GA_stcs_pos[fb].save("{}GA_TF_dics_pos_{}-{}_{}-stc.h5".format(save_dir,freq_bins[fb][0],freq_bins[fb][-1],loi))
GA_fig_p = plot_source_spectrogram(GA_stcs_pos, freq_bins, tmin=tmin_plot, tmax=tmax_plot, source_index=None, colorbar=True, show=False)    # freq_bins! or error
GA_fig_p.savefig("{}GA_pos_{}.png".format(plot_dir,loi))

# GA for contrast
for f in range(len(coll_stcs_cont[0])):
    GA_stcs_cont.append(coll_stcs_cont[0][f])
    for s in range(len(coll_stcs_cont[1:])):
        GA_stcs_cont[f] += coll_stcs_cont[s][f]
    GA_stcs_cont[f] /= len(coll_stcs_cont)
for fb in range(len(freq_bins)):
    GA_stcs_cont[fb].save("{}GA_TF_dics_cont_N-P_{}-{}_{}-stc.h5".format(save_dir,freq_bins[fb][0],freq_bins[fb][-1],loi))
GA_fig_c = plot_source_spectrogram(GA_stcs_cont, freq_bins, tmin=tmin_plot, tmax=tmax_plot, source_index=None, colorbar=True, show=False)    # freq_bins! or error
GA_fig_c.savefig("{}GA_cont_{}.png".format(plot_dir,loi))
